...
function initRings() {
	var rings = document.querySelectorAll('.rings-svg .ring[data-count]');
	if (!rings.length) return;
	var total = 0;
	rings.forEach(function(r) {
		total += parseInt(r.dataset.count) || 0;
	});
	if (total === 0) total = 1;
	rings.forEach(function(ring) {
		var r = parseFloat(ring.getAttribute('r'));
		var circumference = 2 * Math.PI * r;
		var count = parseInt(ring.dataset.count) || 0;
...
function calcTotal() {
	...
	if (!el) return;
	var chips = document.querySelectorAll('.tag-chip .chip-count');
	var total = 0;
	if (chips.length) {
		...
		document.querySelectorAll('.chart-col[data-count]').forEach(function(b) {
			total += parseInt(b.dataset.count) || 0;
		});
	}
	animateNumber(el, 0, total, 800);
}
...
function initBarChart() {
	...
	cols.forEach(function(c) {
		chartArea.insertBefore(c, tooltip);
	});
	var max = 0;
	cols.forEach(function(c) {
