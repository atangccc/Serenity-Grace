...
async function fetchGitHubReleases(owner, repo) {
	try {
		const response = await fetchWithTimeout(`${GITHUB_API_BASE}/repos/${owner}/${repo}/releases?per_page=20`, {
			headers: getAuthHeaders()
		});
		if (!response.ok) throw new Error('Failed to fetch');
		return await response.json();
...
function createProjectCard(data, isCustom = false, ownerType = 'mine') {
	...
	const projectUrl = isCustom ? data.url : (data.html_url || data.url);
...
function createReleaseItem(release, isLatest) {
	...
	item.innerHTML = `<div class="release-marker"><div class="release-dot"></div><div class="release-line"></div></div><div class="release-content"><div class="release-header"><span class="release-tag">${release.tag_name}</span>${isLatest?'<span class="release-latest-badge">最新</span>':''}${release.prerelease?'<span class="release-prerelease-badge">预发布</span>':''}</div><h3 class="release-title">${release.name||release.tag_name}</h3><div class="release-meta"><span class="release-date">${dateStr}</span><span class="release-time-ago">${timeAgo}</span></div>${release.body?`<div class="release-body">${formatReleaseBody(release.body)}</div>`:''}<div class="release-actions"><a href="${release.html_url}" target="_blank" rel="noopener noreferrer" class="release-link"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>查看详情</a></div></div>`;
	return item;
}
...
function formatReleaseBody(body) {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (/^###\s+(.+)$/.test(line)) {
			...
			result.push('<h4>' + escapeForHtml(line.replace(/^###\s+/, '')) + '</h4>');
			continue;
		}
		if (/^##\s+(.+)$/.test(line)) {
			if (inList) {
			...
			result.push('<h3>' + escapeForHtml(line.replace(/^##\s+/, '')) + '</h3>');
			continue;
		}
		if (/^#\s+(.+)$/.test(line)) {
			if (inList) {
			...
			result.push('<h2>' + escapeForHtml(line.replace(/^#\s+/, '')) + '</h2>');
			continue;
		}
		if (/^[\*\-]\s+(.+)$/.test(line)) {
			...
			result.push('<li>' + escapeForHtml(line.replace(/^[\*\-]\s+/, '')) + '</li>');
			continue;
		}
		if (/^\d+\.\s+(.+)$/.test(line)) {
			...
			result.push('<li>' + escapeForHtml(line.replace(/^\d+\.\s+/, '')) + '</li>');
			continue;
		}
		if (inList) {
			result.push(listType === 'ol' ? '</ol>' : '</ul>');
		...
		result.push('<p>' + escapeForHtml(line) + '</p>');
	}
	if (inList) result.push(listType === 'ol' ? '</ol>' : '</ul>');
	...
	inlineCodes.forEach((code, i) => {
		html = html.replace(`__INLINE_CODE_${i}__`, `<code>${escapeForHtml(code)}</code>`);
	});
	codeBlocks.forEach((code, i) => {
		const lang = code.split('\n')[0];
		const content = lang ? code.slice(lang.length + 1) : code;
		html = html.replace(`__CODE_BLOCK_${i}__`, `<pre><code>${escapeForHtml(content)}</code></pre>`);
	});
	return html;
}
...
async function loadProjects() {
	...
	document.getElementById('mineProjects').textContent = mineCount;
	document.getElementById('starredProjects').textContent = starredCount;
	document.getElementById('totalStars').textContent = fmt(totalStars);
	document.getElementById('totalForks').textContent = fmt(totalForks);
